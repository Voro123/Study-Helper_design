<template>
  <caption class="home-body">
    <template v-if="h1">
      <div class="jiantou-div" @click="to_prevpage" :class="{active:canprev}">
        <span class="iconfont iconjiantou prevpage"></span>
      </div>
      <div class="container">
      ...
      <div class="jiantou-div" @click="to_nextpage" :class="{active:cannext}">
        <span class="iconfont iconjiantou nextpage"></span>
      </div>
    </template>
  ...
  methods: {
    ...
    h3_delete (data) {
      if (confirm(`确定要删除词条【${data.name}】吗`)) {
        this.emitData('axiosGetData', 'delete/h3', { id: data.id }, function (res) {
          if (!res.data) {
            console.log('词条删除失败了')
          }
...
.home-body {
  .bg {
    ...
    background: url('../assets/images/index.jpg'), linear-gradient(white, #ff88dd);
